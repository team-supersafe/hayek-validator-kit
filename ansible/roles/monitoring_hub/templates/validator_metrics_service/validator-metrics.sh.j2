...
PROCESS_DEBUG=false
PROCESS_DEVNET=false
PROCESS_LOCALNET=false
...
check_rpc_health () {
  ...
  if [ -z "$health" ] || [ "$health" = "null" ]; then
    ...
    if [ -z "$health" ] || [ "$health" = "null" ]; then
      echo "❌ RPC health check failed for $name"; exit 1; fi
  fi
...
check_produced_blocks () {
  ...
  # Obtain leader slots via JSON-RPC for accuracy
  local LEADER_SLOTS_JSON SLOTS_ARRAY_LENGTH SLOTS_LIST
  LEADER_SLOTS_JSON=$(curl -s --max-time 10 "$RPC_URL" -X POST -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1,"method":"getLeaderSchedule","params":[null,{"identity":"'"$IDENTITY_KEY"'"}]}' || true)
  SLOTS_LIST=$(echo "$LEADER_SLOTS_JSON" | jq -r '.result."'"$IDENTITY_KEY"'" | join(" ")' 2>/dev/null || echo "")
  SLOTS_ARRAY_LENGTH=$(echo "$LEADER_SLOTS_JSON" | jq -r '.result."'"$IDENTITY_KEY"'" | length' 2>/dev/null || echo 0)
  [ -n "$SLOTS_LIST" ] || { echo "ℹ️  No leader slots found"; return 0; }
...
collect_all_validator_metrics() {
  ...
  $PROCESS_MAINNET  && send_validator_metrics "$MAINNET_VOTE_ACCOUNT"  "$MAINNET_IDENTITY_KEY"  "$MAINNET_RPC_API"  "$MAINNET_HOST"  "$MAINNET_STAKEWIZ_ENABLED" "$MAINNET_GOSSIP_ENABLED" "$MAINNET_JITO_MEV_ENABLED"
  $PROCESS_TESTNET  && send_validator_metrics "$TESTNET_VOTE_ACCOUNT"  "$TESTNET_IDENTITY_KEY"  "$TESTNET_RPC_API"  "$TESTNET_HOST"  "$TESTNET_STAKEWIZ_ENABLED" "$TESTNET_GOSSIP_ENABLED" "$TESTNET_JITO_MEV_ENABLED"
  $PROCESS_DEBUG    && send_validator_metrics "$DEBUG_VOTE_ACCOUNT"    "$DEBUG_IDENTITY_KEY"    "$DEBUG_RPC_API"    "$DEBUG_HOST"    "$DEBUG_STAKEWIZ_ENABLED" "$DEBUG_GOSSIP_ENABLED" "$DEBUG_JITO_MEV_ENABLED"
  $PROCESS_DEVNET   && send_validator_metrics "$DEVNET_VOTE_ACCOUNT"   "$DEVNET_IDENTITY_KEY"   "$DEVNET_RPC_API"   "$DEVNET_HOST"   "$DEVNET_STAKEWIZ_ENABLED" "$DEVNET_GOSSIP_ENABLED" "$DEVNET_JITO_MEV_ENABLED"
  $PROCESS_LOCALNET && send_validator_metrics "$LOCALNET_VOTE_ACCOUNT" "$LOCALNET_IDENTITY_KEY" "$LOCALNET_RPC_API" "$LOCALNET_HOST" "$LOCALNET_STAKEWIZ_ENABLED" "$LOCALNET_GOSSIP_ENABLED" "$LOCALNET_JITO_MEV_ENABLED"
...
collect_all_block_metrics() {
  ...
  if $PROCESS_DEVNET; then
    echo "🔄 Processing Devnet blocks"
    readarray -t arr <<< "$(check_produced_blocks "$DEVNET_IDENTITY_KEY" "$DEVNET_HOST" devnet)"
    for kv in "${arr[@]}"; do STATS["DEVNET_${kv%%=*}"]="${kv#*=}"; done
    echo
  fi

  if $PROCESS_LOCALNET; then
    echo "🔄 Processing Localnet blocks"
    readarray -t arr <<< "$(check_produced_blocks "$LOCALNET_IDENTITY_KEY" "$LOCALNET_HOST" localnet)"
